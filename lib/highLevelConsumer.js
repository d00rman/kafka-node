...
HighLevelConsumer.prototype.connect = function () {
    ...
    this.checkPartitionOwnershipInterval = setInterval(function () {
        ...
        if (!self.topicPayloads.length) {
            // the consumer owns no partition, do nothing for now
            return;
        }
        ...
        async.each(self.topicPayloads, function (tp, cbb) {
            ...
            if (/^consumer not/i.test(err)) {
                // the partition is no longer owned by this process, a rebalance is needed
                return rebalance();
            }
...
HighLevelConsumer.prototype.rebalanceAttempt = function (oldTopicPayloads, cb) {
    ...
    async.series([
            ...
            // check the old partitions
            function (callback) {
                if (!oldTopicPayloads.length) {
                    return callback();
                }

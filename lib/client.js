...
Client.prototype.connect = function () {
    ...
    zk.once('init', function (brokers) {
        ...
        var b_keys = Object.keys(brokers);
        var b_idx = 0;
        if(b_keys.length > 1) {
            b_idx = Math.floor(Math.random() * b_keys.length);
        }
        var broker = brokers[b_keys[b_idx]];
        var addr = broker.host + ':' + broker.port;
        self.brokers[addr] = self.brokers[addr] || self.createBroker(broker.host, broker.port);
        self.emit('ready');
    });
